...
# panel from the NYGC phased release (tools/1000g-chr22.lock.json by default).
...
source "$ROOT/tools/lib/fetch.sh"

LOCK=$ROOT/tools/1000g-chr22.lock.json OUT= DOWNLOAD=0
while (($#)); do
    case $1 in
    --lock) LOCK=$2; shift 2 ;;
    --out) OUT=$2; shift 2 ;;
...
jq -r '.files[] | [.name, .url, .bytes, (.md5 // ""), (.sha256 // "")] | @tsv' "$LOCK" |
    while IFS=$'\t' read -r name url bytes md5sum_expected sha256_expected; do
        path=$OUT/source/$name
        fetch_file "$path" "$url" "$bytes"
        [[ -z $md5sum_expected || $(md5 "$path") == "$md5sum_expected" ]] || die "MD5 mismatch: $name"
