...
no_symlinks "$ROOT"
[[ ! -e $ROOT || -d $ROOT ]] || die 'Output is not a directory'
...
EXISTING=$ROOT; while [[ ! -e $EXISTING ]]; do EXISTING=$(dirname -- "$EXISTING"); done
...
mkdir -p -- "$ROOT"
no_symlinks "$ROOT"
exec 9>"$ROOT/.acquire.lock"
flock -n 9 || die "Another acquisition holds $ROOT/.acquire.lock"
PART=
cleanup() { [[ -z $PART ]] || rm -f -- "$PART"; rm -f -- "$ROOT/.acquire.lock"; }
trap cleanup EXIT
