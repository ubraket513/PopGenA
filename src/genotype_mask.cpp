...
struct MaskCounts {
    ...
    json report() const {
        return {{"kept", kept}, {"missing", missing}, {"quality", quality},
                {"unsupported", unsupported}, {"masked", missing + quality + unsupported}};
    }
};
...
int quality_values(const bcf_hdr_t* header, bcf1_t* record, const char* name,
                   int threshold, int samples, IntBuffer& values) {
    if (!threshold) return 0;
    int count = bcf_get_format_int32(header, record, name, &values.data, &values.capacity);
    if (count == -1 || count == -3) return 0;
    if (count != samples)
        throw std::runtime_error(std::string("Invalid scalar Integer FORMAT/") + name);
    for (int i = 0; i < count; ++i)
        if (values.data[i] < 0 && values.data[i] != bcf_int32_missing &&
            values.data[i] != bcf_int32_vector_end)
...
json mask_genotypes(const fs::path& input, const fs::path& output, int min_dp, int min_gq) {
    ...
    for (int i = 0; i < samples; ++i) {
        ...
        if (id.empty() || id == "0" || id == "." || id[0] == '#' ||
            id.find_first_of(" \t\r\n\v\f") != std::string::npos ||
            id.find_first_of(std::string(1, '\x7f')) != std::string::npos ||
            !ids.insert(id).second)
            throw std::runtime_error("Sample IDs must be unique, nonmissing PLINK IDs without whitespace");
        for (unsigned char c : id) if (c < 32) throw std::runtime_error("Control character in sample ID");
    }
    ...
    if (!bcf_hdr_idinfo_exists(header.get(), BCF_HL_FMT, gt_id)) {
        if (bcf_hdr_append(header.get(), "##FORMAT=<ID=GT,Number=1,Type=String,Description=\"Genotype\">") != 0 ||
            bcf_hdr_sync(header.get()) != 0) throw std::runtime_error("Cannot add GT header");
    ...
    while ((status = bcf_read(in.get(), header.get(), record.get())) == 0) {
        ++records;
        if (record->errcode || bcf_unpack(record.get(), BCF_UN_ALL) < 0 ||
            record->rid < 0 || record->rid >= header->n[BCF_DT_CTG] || record->pos < 0 ||
        ...
        previous_rid = record->rid; previous_pos = record->pos;
        const char* chrom = bcf_hdr_id2name(header.get(), record->rid);
        if (!chrom || !autosome(chrom)) { ++non_autosomal; continue; }
        ...
        if (record->n_allele != 2 || !base(record->d.allele[0]) || !base(record->d.allele[1]) ||
            record->d.allele[0][0] == record->d.allele[1][0]) { ++non_snp; continue; }
        bool pass = true;
        ...
        for (int i = 0; i < samples; ++i) {
            ...
            for (int j = 0; j < stride; ++j) {
                ...
                if (value == bcf_int32_vector_end) { ended = true; continue; }
                if (ended || value < 0) throw std::runtime_error("Malformed GT encoding");
                ++ploidy;
                if (bcf_gt_is_missing(value)) missing = true;
                else if (bcf_gt_allele(value) > 1) throw std::runtime_error("GT allele exceeds biallelic REF/ALT range");
            }
            auto& c = sample_counts[i];
            bool keep = false;
            if (missing || ploidy == 0) { ++c.missing; ++total.missing; }
            else if (ploidy != 2) { ++c.unsupported; ++total.unsupported; }
            else if (!passes(min_dp, ndp, dp, i) || !passes(min_gq, ngq, gq, i)) { ++c.quality; ++total.quality; }
            else { ++c.kept; ++total.kept; keep = true; }
            for (int j = 0; j < 2; ++j)
                masked[static_cast<size_t>(i) * 2 + j] = keep ? gt.data[static_cast<size_t>(i) * stride + j] : bcf_gt_missing;
        }
        ...
        if (bcf_write(out.get(), header.get(), record.get()) != 0) throw std::runtime_error("Cannot write masked BCF record");
        ++kept;
    }
    ...
    // Windows filesystem rename fails if its destination already exists.
    if (exists_entry(target) || exists_entry(index)) throw std::runtime_error("Mask output appeared during processing");
    fs::rename(staged_bcf, target);
    fs::rename(staged_csi, index);
    fs::remove(stage);
    json per_sample = json::array();
    for (int i = 0; i < samples; ++i) {
        auto c = sample_counts[i].report(); c["sample"] = header->samples[i]; per_sample.push_back(std::move(c));
    }
    return {{"schema_version", 1}, {"status", "complete"}, {"records", records}, {"kept", kept},
        {"sample_count", samples}, {"min_dp", min_dp}, {"min_gq", min_gq},
        {"skipped", {{"non_autosomal", non_autosomal}, {"not_biallelic_snp", non_snp}, {"site_filter", filtered}}},
        {"genotypes", total.report()}, {"samples", std::move(per_sample)},
        {"removed_info", {"AC", "AN", "AF", "NS"}}, {"output", utf8(target)}, {"index", utf8(index)}};
}
}
