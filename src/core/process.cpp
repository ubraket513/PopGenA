...
UniqueFd open_output(const fs::path& path) {
    UniqueFd fd(::open(path.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644));
    if (fd.get() < 0) throw_errno("Open process output " + utf8(path));
    return fd;
}
...
void install_cancellation_handler(bool follow_parent) {
    ...
    if (follow_parent && ::prctl(PR_SET_PDEATHSIG, SIGTERM) != 0) throw_errno("Request parent-death signal");
    struct sigaction action{};
    action.sa_handler = on_cancel;
    sigemptyset(&action.sa_mask);
    for (int signal : {SIGINT, SIGTERM, SIGHUP})
        if (sigaction(signal, &action, nullptr) != 0) throw_errno("Install cancellation handler");
}
...
ProcessResult run_pipeline(const std::vector<Command>& commands, const ProcessOptions& o) {
    ...
    if (null_input.get() < 0) throw_errno("Open /dev/null");
    ...
    try {
        ...
        for (size_t i = 0; i < stages.size(); ++i) {
            ...
            if (!last && ::pipe2(pipe_fds, O_CLOEXEC) != 0) throw_errno("Create pipe");
            int report[2];
            if (::pipe2(report, O_CLOEXEC) != 0) throw_errno("Create exec status pipe");
            pid_t pid = ::fork();
            ...
            if (n == sizeof child_errno) {
                if (previous >= 0) ::close(previous);
                errno = child_errno;
                throw_errno("Launch " + stages[i].args.front());
            }
        }
        ...
        while (remaining) {
            for (size_t i = 0; i < pids.size(); ++i) {
                ...
                if (r < 0 && errno != EINTR) throw_errno("Wait for " + stages[i].args.front());
