...
json select_cohort(const fs::path& original,const fs::path& psam,const fs::path& kinship,
                   const fs::path& metadata,const fs::path& out,const std::string& policy,double threshold);
