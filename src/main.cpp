...
int main() {
    ...
    std::string raw1,raw2,raw_input,raw_out,raw_hash,raw_assembly,raw_reference,raw_metadata,fastp_report,fastp_before,fastp_out;
    uint64_t raw_bases=0;std::vector<std::string> raw_bams,raw_samples,raw_libraries;
    auto* raw_fastq=app.add_subcommand("raw-fastq","Internal: stream-validate paired Phred+33 FASTQ");
    raw_fastq->add_option("--read1",raw1)->required();raw_fastq->add_option("--read2",raw2)->required();
    raw_fastq->add_option("--fastp-report",fastp_report);raw_fastq->add_option("--before",fastp_before);raw_fastq->add_option("--report-out",fastp_out);
    auto* raw_ref=app.add_subcommand("raw-reference","Internal: verify, copy and index the exact reference");
    raw_ref->add_option("--input",raw_input)->required();raw_ref->add_option("--out",raw_out)->required();raw_ref->add_option("--sha256",raw_hash)->required();raw_ref->add_option("--max-bases",raw_bases)->required();raw_ref->add_option("--assembly",raw_assembly)->required();
    auto* raw_bam=app.add_subcommand("raw-bams","Internal: validate reference, read groups and indexed BAMs before joint calling");
    raw_bam->add_option("--bam",raw_bams)->required();raw_bam->add_option("--sample",raw_samples)->required();raw_bam->add_option("--library",raw_libraries)->required();
    raw_bam->add_option("--reference",raw_reference)->required();raw_bam->add_option("--samples",raw_metadata);raw_bam->add_option("--out",raw_out)->required();
    ...
    try {
        ...
        if(*raw_fastq){auto report=pg::check_fastq_pair(pg::from_utf8(raw1),pg::from_utf8(raw2));if(!fastp_report.empty())report["fastp_report"]=pg::normalize_fastp_report(pg::from_utf8(fastp_report),pg::from_utf8(fastp_before),report,pg::from_utf8(fastp_out));std::cout<<report.dump(2)<<'\n';return 0;}
        if(*raw_ref){auto report=pg::prepare_reference(pg::from_utf8(raw_input),pg::from_utf8(raw_out),raw_hash,raw_bases);report["assembly"]=raw_assembly;std::cout<<report.dump(2)<<'\n';return 0;}
        if(*raw_bam){std::cout<<pg::check_alignments(raw_bams,raw_samples,raw_libraries,pg::from_utf8(raw_reference),pg::from_utf8(raw_metadata),pg::from_utf8(raw_out)).dump(2)<<'\n';return 0;}
