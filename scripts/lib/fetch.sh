...
# split_ranges <cache> <suffix> [chunk-bytes]: read "start end" lines on stdin and print
# requests of at most chunk-bytes (default 1 MiB) as "start end path".
split_ranges() {
    awk -v cache="$1" -v suffix="$2" -v chunk="${3:-1048576}" '{
        for (s = $1; s <= $2; s += chunk) {
            e = s + chunk - 1; if (e > $2) e = $2
            printf "%.0f %.0f %s/%.0f-%.0f.%s\n", s, e, cache, s, e, suffix
        } }'
}

# fetch_ranges <url> <total-size regex> <requests>: fetch missing "start end path" ranges
# (FETCH_PARALLEL at a time, default 4), check each Content-Range exactly, record SHA256s.
fetch_ranges() {
...
# fetch_file_ranged <path> <url> <bytes> <cache>: whole-file download as parallel 8 MiB
# ranges (resumable, for servers that throttle single connections), then assembled.
fetch_file_ranged() {
    local path=$1 url=$2 bytes=$3 cache=$4 requests
    [[ -e $path ]] && return 0
    requests=$(printf '0 %s\n' "$((bytes - 1))" | split_ranges "$cache" part 8388608)
    FETCH_PARALLEL=${FETCH_PARALLEL:-8} fetch_ranges "$url" "$bytes" "$requests"
    while read -r _ _ chunk; do cat -- "$chunk"; done <<<"$requests" >"$path.part"
    [[ $(stat -c %s -- "$path.part") == "$bytes" ]] || die "Assembled size differs for $url"
    mv -- "$path.part" "$path"
}
