...
demo() {
    jq -n --arg work "$1" --arg cohort "$TEST_ROOT/cohort.vcf" --arg samples "$PROJECT/tests/data/fixtures/samples.tsv" --arg bcftools "$BCFTOOLS" '{
...
cp "$PROJECT/tests/data/fixtures/cohort.vcf" "$TEST_ROOT/cohort.vcf"
...
check 'workflow output differs from independent golden' cmp -s "$(result_dir "$WORK" stats)/samples.tsv" "$PROJECT/tests/data/golden/samples.tsv"
