...
VCF=$PROJECT/tests/fixtures/cohort.vcf
META=$PROJECT/tests/fixtures/samples.tsv
...
for name in samples.tsv populations.tsv; do
    check "independent golden mismatch: $name" cmp -s "$TEST_ROOT/plain/$name" "$PROJECT/tests/golden/$name"
done
